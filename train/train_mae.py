...
parser.add_argument('--datasets', type=list, default=['CRWU', 'HUST', 'LinGang'],
        ...
        if epoch % 10 == 0:
            torch.save(
                {'epoch': epoch,
                 ...
                 os.path.join(args.savepath, 'MAE/checkpoint_e%d.pth' % (epoch)))
            if args.visualize:
                # visualize
                visualize(visdataloader, model, 
                          savefolder=os.path.join(args.savepath, f"MAE/pics/epoch{epoch}/"), 
                          patchnum=patchnum,
                          mask_ratio=mask_ratio, num=30)

        # save training
        with open(os.path.join(args.savepath, 'MAE/logs.txt'), 'a') as file:
